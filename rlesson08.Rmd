...
Compute $E[UV]$ and $E[U+V]$ via simulation.
...
Determine $\rho\left(U,U^2\right)$ when $U\sim\text{U}(0,a)$ for $a=2$ and $a=200$. In class, we computed the value to be $\sqrt{13}/4$= `r sqrt(15)/4` for any $a>0$
...
ggplot() +
  theme_classic() +
  geom_point(aes(x = u, y = u^2), size = 1) +
  labs(x = "U", y = expression(U^2), 
       title = expression(paste("25 simulated values of U and ", U^2))) + 
...
ggplot() +
  theme_classic() +
  geom_point(aes(x = x, y = x^2), size = 1) +
  labs(x = "X", y = "Y", title = "25 simulated values of U and Y") + 
  theme(
    axis.text = element_text(size = 14),
    axis.title = element_text(size = 14),
    plot.title = element_text(size = 18)
